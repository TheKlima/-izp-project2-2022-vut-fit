...
## Prémiové hodnocení (přidáno 2022-11-22)

Prémiové hodnocení je dobrovolné a lze za něj získat bonusové body. Podmínkou pro udělení prémiových bodů je výborné vypracování standardního zadání. Výsledné hodnocení je plně v kompetenci vyučujího, který bude projekt hodnotit. Výše prémiových bodů závisí také na sofistikovanosti řešení. Prémiové řešení by mělo obsahovat další algoritmy shlukových analýz (k-means, případně i další, např. metodu nejvzdálenějšího souseda). Pro volbu metody při spuštění programu definujte oddělený nepovinný parametr (např. -k).
